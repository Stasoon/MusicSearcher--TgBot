...
def __get_song_file(song: VkSong) -> InputFile | str:
    file_id = songs_hashes.get_song_file_id(song_id=song.audio_id, owner_id=song.owner_id)
...
async def handle_show_song_callback(callback: CallbackQuery, callback_data: ShowSongCallback):
    ...
    song = await VkMusicApi.get_song_by_id(song_id=song_id, owner_id=owner_id)
    ...
    try:
        ...
        songs_hashes.save_song_if_not_hashed(
            song_id=song.audio_id, owner_id=song.owner_id, file_id=file_msg.audio.file_id
        )
