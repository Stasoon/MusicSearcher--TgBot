...
async def __send_found_songs(message: Message, query: str, found_count: int, songs: list[Song], caption: str):
    if not songs:
        ...
        await message.bot.send_message(
            chat_id=-1002043176809,
            text=f'Песня не найдена. \nЗапрос: {query} \nПользователь: {message.from_user}',
            message_thread_id=2
        )
        return
...
async def handle_chat_text_message(message: Message):
    query = message.text.lower().replace('найти', '', 1).strip()
    found_count, _, songs_from_cache = get_cached_songs_for_request(q=query, count=SONGS_PER_PAGE, offset=0)
    ...
    counter, _ = ChatSearchStats.get_or_create(date=datetime.date.today())
    counter.downloads_count += 1
    counter.save()
...
async def handle_youtube_link(message: Message):
    ...
    try:
        ...
        logger.error(e)
        await message.answer(UserMessages.get_song_not_found_error())
        return
...
async def handle_show_song_callback(callback: CallbackQuery, callback_data: SongCallback):
    ...
    counter, _ = DownloadsByDays.get_or_create(date=datetime.date.today())
    counter.downloads_count += 1
    counter.save()
