...
from src.database.reflinks import create_reflink, is_reflink_exists, get_all_links, get_link, delete_reflink
...
class Messages:
    @staticmethod
    def get_referral_links() -> str:
        text = '🔗 Реферальные ссылки \n\n' \
               '<b>Список реферальных ссылок:</b> \n\n'

        for n, link in enumerate(get_all_links(), 1):
            text += f'{n} — <code>{link.name}</code> \n'
        text += '-' * 30
        return text
...
class Handlers:
    ...
    async def __handle_admin_reflinks_button(message: Message):
        await message.answer(Messages.get_referral_links(), reply_markup=Keyboards.reflinks_markup)
    ...
    async def __handle_cancel_callback(callback: CallbackQuery, state: FSMContext):
        await callback.message.edit_text(Messages.get_referral_links(), reply_markup=Keyboards.reflinks_markup)
        await state.finish()
